...
2. **Preliminary check**: Verify that a PBIP canvas exists in the project folder (`<ProjectName>/PBIP/*.pbip`). If not, STOP and instruct the user to create it first.
3. **Prerequisites check**: Verify Python 3.10+ is available and `.venv` exists at repository root. If not, guide setup.
4. Follow the 7-step workflow defined in `.github/agents/semantic-modeler.agent.md` **exactly and sequentially**.
...
- Reference a `.md` file in the project input folder (e.g., `<ProjectName>/input/spec_sales_overview.md`)
