...
> **CRITICAL**: Temporary inspection artifacts used to study Microsoft schemas or Desktop output are not part of the PBIP project structure. They must not remain in the repository root or inside project folders as if they were PBIP assets. Any reusable conclusion learned from them must be moved into `.github/references/` or the relevant skill file in generalized form.
...
Operational rule:
- Scripts and validators must resolve `<PbipBaseName>` from the actual `.pbip` artifact or from the `PBIP/` folder contents. They must not assume `<PbipBaseName>` equals `<ProjectName>`.
...
Use these rules when inspecting or manipulating PBIP/PBIR structure:

- Project artifacts belong under the project folder, typically within `PBIP/`, `data/`, `scripts/`, `tests/`, and `spec/`.
- Repository knowledge belongs under `.github/skills/` and `.github/references/`.
- One-off scratch files created only to inspect external schemas or temporary payloads are not project assets and should be deleted after use.
- If a structural rule is generic, store it in a reference file instead of leaving it embedded only in a generated project artifact or in chat history.
