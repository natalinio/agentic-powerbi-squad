...
description: >-
  Use when the task involves writing, modifying, or reviewing DAX measures in a Power BI semantic model.
  Triggers: "write a DAX measure", "add YTD measure", "create time intelligence", "DAX formula",
  "FYTD calculation", "variance measure", "percentage measure", "fix DAX error",
  "DAX performance", "DIVIDE safety", "VAR/RETURN pattern", "display folder",
  "formatString for measure", "conditional formatting measure", "budget vs actuals".
user-invocable: true
---
...
1. **Read** `.github/skills/dax-development/references/dax-patterns.md` for validated DAX patterns.
2. **Read** `.github/skills/dax-development/references/dax-pitfalls.md` — anti-hallucination guard for deprecated, non-existent, and commonly confused DAX functions.
3. **Read** `.github/references/naming-conventions.md` for measure naming rules.
4. **Search** Microsoft documentation with `microsoft_docs_search` for any DAX function you are uncertain about.
5. **Search** for code examples with `microsoft_code_sample_search` when implementing time intelligence or complex calculations.
...
### TMDL Descriptions (`///`)
- Use `///` (triple-slash) before a measure declaration to set its `Description` property.
